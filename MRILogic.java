...
public class MRILogic implements GameLogic<Integer> {
    ...
    private int movePawn(Player player, int x, int y) {
        ...
        if (value != -1) {
            ...
            int cuop= pawn.getCurrentOptional();
            if (current == 74)
                player.incrementCoin();
            
            if((dice.content==2||dice.content==4||dice.content==6)&& pawn.getop()==1){
                opcionalinicial(pawn,current);
                pawn.op(2);
            }else if (pawn.getop()==1){
                pawn.op(0);
            }
            if((pawn.getCurrentOptional() > 8)&& pawn.getop()==2){
                opcionalfinal(pawn, current);
                current = pawn.getCurrent();
                pawn.op(0);
            }
            
            if(((current == 3)||(current == 20)||(current == 37)||(current == 54))&&pawn.getop()==0){
                pawn.op(1);
            }
    ...
    public void opcionalinicial(Pawn pawn, int current){
        ...
        for(int i = 0; i < 4; i++){
            if(j == i){
                ...
                int mm= pawn.getCurrent();
                pawn.setCurrentOptional(dice.content);
                break;
            }
        }
    }
    public void opcionalfinal(Pawn pawn, int current){
        pawn.setPathOptional(false);
        for(int i = 0; i < 4; i++){
    ...
    public boolean getdoubleClicked(){return false;}
