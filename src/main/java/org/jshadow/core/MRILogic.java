...
public class MRILogic implements GameLogic<Integer> {
     ...
     */
    public MRILogic() {
        board = new MRIBoard(new Position(80, 50));
        currentPlayer = 1;
        dice = new Dice<>("int");
        dice.content = 0;
        flag = 0;
    ...
    private int movePawn(Player player, int x, int y) {
        ...
        if (value != -1) {
            ...
            // safeZone: * alternative path here is missing
            if ((current % 17) != 0 && current < 67) {
                for (int i = 1; i <= 4; i++) {
    ...
    public void paint(Graphics2D graphics) {
        ...
        if (player.getCoin() == 4) {
            ...
            graphics.drawString( "Player " + currentPlayer + " wins.", 690, 150);
            ...
            graphics.setColor(Color.WHITE);
            graphics.fillRect(680, 10, 400, 200);
            graphics.setColor(player.getColor());
            graphics.setFont(new Font("serif", Font.BOLD, 40));
            graphics.drawString( "Player " + currentPlayer, 690, 150);
            graphics.drawString( "Number on dice is " + dice.content, 690, 200);
        }
